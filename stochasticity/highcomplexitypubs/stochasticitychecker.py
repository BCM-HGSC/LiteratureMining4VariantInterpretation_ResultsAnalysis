...
DEFAULT_INPUT_FILES = {
    "gpt5high":   str(INPUT_DIR / "v7_gpt5high_variant_summary__det_detsetonly.csv"),
    "claude":     str(INPUT_DIR / "v7_claude_variant_summary__det_detsetonly.csv"),
    "gemini":     str(INPUT_DIR / "v7_gemini_variant_summary__det_detsetonly.csv"),
    "o4minihigh": str(INPUT_DIR / "v7_o4minihigh_variant_summary__det_detsetonly.csv"),
    "o3":         str(INPUT_DIR / "v7_o3_variant_summary__det_detsetonly.csv"),
}
...
def analyze_files(input_files, embed_model, thresholds, out_prefix):
    ...
    res.to_csv(OUTPUT_DIR / "stochasticity_only_per_pmid.csv", index=False)
    summary.to_csv(OUTPUT_DIR / "stochasticity_only_summary_by_model.csv", index=False)
    return res, summary

# ------------ Run ------------
if __name__ == "__main__":
    res, summary = analyze_files(DEFAULT_INPUT_FILES, DEFAULT_EMBED_MODEL, DEFAULT_THRESHOLDS, "ps4_stochasticity")
    print(
        "Wrote:\n - "
        + str(OUTPUT_DIR / "stochasticity_only_per_pmid.csv")
        + "\n - "
        + str(OUTPUT_DIR / "stochasticity_only_summary_by_model.csv")
    )
    print(summary)



#sanity test - python stochasticitychecker.py --test --model gpt5high --pmid 31342592 --plots

#full test - python stochasticitychecker.py --plots
